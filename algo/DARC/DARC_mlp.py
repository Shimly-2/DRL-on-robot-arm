...
import copy
from algo.DARC.net_mlp import PolicyNet, QValueNet
...
class DARC_MLP:
    ...
    def __init__(self, state_dim, action_dim, action_bound, hidden_dim=opt.hidden_dim,
                 actor_lr=opt.actor_lr, critic_lr=opt.critic_lr, sigma=opt.sigma, tau=opt.tau, gamma=opt.gamma, policy_noise=opt.policy_noise, noise_clip=opt.noise_clip,
                 policy_freq=opt.policy_freq, q_weight=opt.q_weight, regularization_weight=opt.regularization_weight, device=opt.device):
        '''
        用于初始化TD3算法中的各项参数，
        初始化策略网络与估值网络
    ...
    def update(self, transition_dict, update_a1=True, batch_size=100):
        ...
        target_Q = self.q_weight * torch.min(target_Q1, target_Q2) + (1.0 - self.q_weight) * torch.max(target_Q1, target_Q2)
